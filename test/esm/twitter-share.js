const observedAttributes = ['text', 'url', 'hashtags', 'via', 'related'];
  ...
  return (
    ...
    observedAttributes
      .reduce((qs, curr) => {
        ...
        if (val) qs.push(curr + '=' + encodeURIComponent(val));
        return qs;
      }, [])
      .join('&')
  );
...
export default {
  ...
  onSSRConnected() {
    ...
    //  * lighter SSR
    // 🎉
    this.attributes = this.attributes.filter((attr) => !observedAttributes.includes(attr.name));
  },
};
